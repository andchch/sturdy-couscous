...
class SteamService:
    ...
    async def get_profile(self, steam_id: str, user_id: int):
        ...
        if profile:
            await self.redis.setex(cache_key, 86400, json.dumps(profile))
        return profile
        ...
        if sql_profile:
            ...
            await self.redis.setex(cache_key, 86400, json.dumps(profile_data))
            return profile_data
    ...
    async def get_games(self, steam_id: str):
        ...
        if games:
            await self.redis.setex(cache_key, 86400, json.dumps(games))
        return games
    ...
    async def get_achievements(self, steam_id: str):
        ...
        if achievements:
            await self.redis.setex(cache_key, 86400, json.dumps(achievements))
        return achievements
    ...
    async def get_friends(self, steam_id: str):
        ...
        if friends:
            await self.redis.setex(cache_key, 86400, json.dumps(friends))
        return friends
