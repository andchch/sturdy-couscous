...
from backend.api_v1.communities.schemes import CommunityCreate, CommunityCreateResponse, CommunityMessageResponse, CommunityListResponse, EditCommunity, GetCommunity
...
@community_router.delete('/{community_id}/leave', response_model=CommunityMessageResponse)
async def leave_community(community_id: int, current_user: Annotated[User, Depends(get_current_user)]):
    # Проверяем, существует ли сообщество
    community = await CommunityDAO.get_by_id(community_id)
    if not community:
        raise community_not_found
    ...
    existing_membership = await CommunityMembershipDAO.get_user_membership(user_id=current_user.id,
                                                                     community_id=community_id)
    if existing_membership:
        await CommunityMembershipDAO.leave(user_id=current_user.id,
                                           community_id=community_id)

    return {'message': f'User {current_user.id} leaved community {community.name}'}
...
@community_router.get('/{community_id}', response_model=GetCommunity)
async def get_community(community_id: int,
    ...
    return await serialize_full_community(community, rediska)
